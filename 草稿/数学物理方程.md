...
\end{cases}
$$可以将原方程代为$p^{2}\tilde{U}+a^{2}k^{2}\tilde{U}=\tilde{F}+p\Phi+\Psi$
...
最后进行Fourier变换的逆变换得：$u(x,t)=\int _{0}^{t} \dfrac{1}{2\sqrt{ 2\pi }a}\int_{-\infty}^{\infty} f(\xi,\tau)\eta(a(t-\tau)-|x-\xi|) \, \mathrm{d}\xi \mathrm{d\tau}+\dfrac{1}{2\sqrt{ 2\pi }}\int_{-\infty}^{\infty} \psi(\xi)\eta(at-|x-\xi|) \, d\xi+\dfrac{1}{2\sqrt{ 2\pi }}\int_{-\infty}^{\infty} \phi(\xi)[\delta(-(x-\xi)+ak)+\delta((x-\xi)+ak)] \, \mathrm{d}\xi$
注：本题计算不一定对
