...
最后进行Fourier变换的逆变换得：$u(x,t)=\int _{0}^{t} \dfrac{1}{2\sqrt{ 2\pi }a}\int_{-\infty}^{\infty} f(\xi,\tau)\eta(a(t-\tau)-|x-\xi|) \, \mathrm{d}\xi \mathrm{d\tau}+\dfrac{1}{2\sqrt{ 2\pi }}\int_{-\infty}^{\infty} \psi(\xi)\eta(at-|x-\xi|) \, d\xi+\dfrac{1}{2\sqrt{ 2\pi }}\int_{-\infty}^{\infty} \phi(\xi) \, dx$
