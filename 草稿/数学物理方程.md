...
第一步，定义该方程下的本征值问题
对于方程$\dfrac{1}{r}\dfrac{ \partial  }{ \partial r }(r\dfrac{ \partial u }{ \partial r })+\dfrac{1}{r^{2}}\dfrac{ \partial^{2} u }{ \partial \phi^{2} }=0$，采用分离变量法可得：$u(r,\phi)=R(r)\Phi(\phi)$
代入后可以得到$r^{2}R^{\prime\prime}+rR^{\prime})$
