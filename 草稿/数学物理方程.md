...
再进行Laplace变换的反演得：$U=\int _{0}^{t}\dfrac{1}{ak}F(k,\tau)\sin ak(t-\tau) \, \mathrm{d}\tau+\dfrac{\Psi}{ak}\sin akt+\Phi \cos akt$
最后进行Fourier变换的逆变换得：$u(x,t)=\int _{0}^{t} \dfrac{1}{2\sqrt{ 2\pi }a}f(\xi,\tau)\eta(a(t-\tau)-|x-\xi|) \, \mathrm{d}\xi+\dfrac{1}{\sqrt{ 2\pi }}$
