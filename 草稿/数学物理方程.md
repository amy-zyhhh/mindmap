...
再进行Laplace变换的反演得：$U=\int _{0}^{t}\dfrac{1}{ak}F(\tau)\sin ak(t-\tau) \, \mathrm{d}\tau+\dfrac{\psi}{ak}\sin akt+\Phi \cos akt$
最后进行Fourier变换的逆变换得：$u(x,t)=\int _{0}^{t} \dfrac{1}{a} \, \mathrm{d}x$
