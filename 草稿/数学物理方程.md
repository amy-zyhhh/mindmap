...
g(t;\tau)=\begin{cases}
0&t<\tau \\
-\dfrac{y_{2}(\tau)y_{1}(\tau)}{p(\tau)W}+\dfrac{y_{1}(\tau)y_{2}(\tau)}{p(\tau)W}&t>\tau 
\end{cases}=\dfrac{-y_{2}(\tau)y_{1}(\tau)+y_{1}(\tau)y_{2}(\tau)}{p(\tau)W}\eta(t-\tau)
$$由此可得，方程的通解为： $$
g(t;\tau)=c_{5}(\tau)y_{1}(t)+c_{6}(\tau)y_{2}(\tau)+\dfrac{-y_{2}(\tau)y_{1}(\tau)+y_{1}(\tau)y_{2}(\tau)}{p(\tau)W}\eta(t-\tau)
$$
第三步，求出方程在边界条件下的特解。
代入边界条件，得： $$
